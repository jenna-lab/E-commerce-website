  ...
  fetch("product.html")
    ...
    .then(function (html) {
      ...
      if (productCategoryElement) {
        productCategoryElement.innerHTML = "Category: ".concat(
          product.category
        );
      }
      ...
      var productDescriptionElements = document.querySelectorAll(
        "#productDescription"
      );
      ...
      if (productRatingElement) {
        productRatingElement.innerText = ""
          .concat(product.rating.rate, " (")
          .concat(product.rating.count, " reviews)");
      }
...
var addToCart = function (product_id) {
  ...
  var CATEGORY_PRODUCT_API = "https://fakestoreapi.com/products/".concat(
    product_id
  );
      ...
      if (found) {
        currentCart = currentCart.map(function (item) {
          return item.id === json.id
            ? _assign(_assign({}, item), { count: (item.count || 1) + 1 })
            : item;
        });
      } else {
        currentCart.push(_assign(_assign({}, json), { count: 1 }));
      }
...
function displayCategory(categories, selectedCategory) {
  console.log(`clicked ${categories} of type ${typeof categories}`);
  ...
  if (navUl) {
    ...
    if (ul_1) {
      categories.forEach(function (category) {
        ...
        a.onclick = function () {
          return displayCategoryProducts(categories);
        };
